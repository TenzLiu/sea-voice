...
public class ShapeImageView extends ImageView {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mTextSize = ta.getDimensionPixelSize(R.styleable.ShapeImageView_textSize, sp2px(context, DEFAULT_TEXT_SIZE));
        ...
        mRadius = ta.getDimensionPixelSize(R.styleable.ShapeImageView_siRadius, DEFAULT_RADIUS);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (getDrawable() != null && TextUtils.isEmpty(getText()) && isAtMost) {
            Bitmap src = getBitmapFromDrawable(getDrawable());
    ...
    private void drawBitmap(Canvas canvas, Shape shape) {
        if (getDrawable() == null) {
            return;
        }else if(getBitmapFromDrawable(getDrawable()) == null){
            return;
        }
        canvas.save();
        Bitmap src = getBitmapFromDrawable(getDrawable());
    ...
    /**
     * 获取bitmap，处理glide设置的DrawableTransitionOptions的转换类型问题
     * @param drawable
     * @return
     */
    ...
    private Bitmap zoom(Bitmap bitmap, int targetWidth, int targetHeight) {
    ...
    private float getScaleVal(Bitmap bitmap, float targetWidth, float targetHeight) {
        if (bitmap == null) {
            return 0;
        }
        try {
